...
const ProductDetails = () => {
  ...
  const handleToggleFavorite = (e, relatedProduct) => {
    ...
    const favoriteProduct = {
      ...
      name: relatedProduct.name?.[lang] || relatedProduct.name?.en || relatedProduct.name,
      price: relatedProduct.bestPriceAfterDiscount || relatedProduct.basePrice || relatedProduct.price,
      image: relatedProduct.images?.[0]?.url,
      category: relatedProduct.category?.name?.[lang] || relatedProduct.category?.name?.en || relatedProduct.category?.name,
      brand: relatedProduct.brand?.name?.[lang] || relatedProduct.brand?.name?.en || relatedProduct.brand?.name,
      description: relatedProduct.description
    };

    if (isInFavorites(relatedProduct._id || relatedProduct.id)) {
      removeFromFavorites(relatedProduct._id || relatedProduct.id, favoriteProduct.name);
    ...
    const fetchProduct = async () => {
      try {
        ...
        const relatedResponse = await fetch(
          `http://127.0.0.1:3000/api/product?categorySlug=${encodeURIComponent(categoryParam)}`
        );
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] w-full py-16" style={{ background: 'var(--primary-bg)' }}>
        ...
        <p className="text-lg" style={{ color: 'var(--secondary-text)' }}>{t("productDetails.loading")}</p>
      </div>
    );
  }
  ...
  if (!product) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] w-full py-16" style={{ background: 'var(--primary-bg)' }}>
        <h2 className="text-2xl font-bold mb-2" style={{ color: 'var(--primary-text)' }}>{t("productDetails.notFound")}</h2>
        <p className="mb-6" style={{ color: 'var(--secondary-text)' }}>{t("productDetails.notFoundDescription")}</p>
        <button 
          onClick={() => navigate("/")} 
  ...
  return (
    <div className="max-w-[1200px] mx-auto px-4 py-10 w-full" style={{ background: 'var(--primary-bg)' }}>
      ...
      <div className="flex items-center gap-2 text-sm mb-8" style={{ color: 'var(--secondary-text)' }}>
        <span onClick={() => navigate("/")} className="cursor-pointer hover:text-blue-600 transition-colors">
          {t("navigation.home")}
        </span>
        <span>/</span>
        <span
      ...
      <section className="max-w-7xl mx-auto px-4 py-10 grid grid-cols-1 lg:grid-cols-2 gap-12 items-start">
        ...
        <div className="flex flex-col items-center justify-center w-full">
          <div className="relative w-full max-w-md aspect-square bg-gradient-to-br from-[rgba(37,99,235,0.05)] to-[rgba(16,185,129,0.03)] rounded-2xl shadow-xl border border-gray-200 dark:border-gray-700 flex items-center justify-center overflow-hidden mb-6">
            ...
            <button
              onClick={(e) => {
                ...
                const favoriteProduct = {
                  ...
                  category: product.category?.name?.[lang] || product.category?.name?.en || product.category?.name,
                  brand: product.brand?.name?.[lang] || product.brand?.name?.en || product.brand?.name,
                  description: product.description
                };

                if (isInFavorites(product._id || product.id)) {
                  removeFromFavorites(product._id || product.id, favoriteProduct.name);
                  toast.success('Removed from favorites');
                } else {
                  addToFavorites(favoriteProduct);
                  toast.success('Added to favorites');
            ...
            >
              <svg className="w-5 h-5" fill={isInFavorites(product._id || product.id) ? "currentColor" : "none"} stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
            ...
            <div className="flex gap-2 justify-center w-full">
              {mainImages.map((image, index) => (
                ...
                >
                  <img 
                    src={image.url} 
                    alt={`${product.name?.[lang] || product.name?.en || product.name} ${index + 1}`}
        ...
        <div className="flex flex-col gap-6 w-full">
          ...
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-sm text-gray-400">({product.averageRating || 0})</span>
          ...
          <p className="text-base mb-4" style={{ color: 'var(--secondary-text)' }}>
            {cleanHtmlTags(product.details?.[lang] ||
          ...
          <div className="flex items-center gap-4 mb-6">
            ...
            <button
              ...
              style={{ 
                background: addToCartSuccess ? '#10b981' : 'var(--gradient-primary)',
          ...
          <div className="grid grid-cols-2 gap-4 mt-6">
            <div className="flex items-center gap-3 p-3 rounded-lg" style={{ background: 'var(--card-bg)' }}>
              ...
              <span className="text-sm font-medium" style={{ color: 'var(--primary-text)' }}>
                {t("productDetails.features.freeShipping")}
              </span>
            </div>
            <div className="flex items-center gap-3 p-3 rounded-lg" style={{ background: 'var(--card-bg)' }}>
              ...
              <span className="text-sm font-medium" style={{ color: 'var(--primary-text)' }}>
                {t("productDetails.features.returns")}
              </span>
            </div>
            <div className="flex items-center gap-3 p-3 rounded-lg" style={{ background: 'var(--card-bg)' }}>
            ...
            <div className="flex items-center gap-3 p-3 rounded-lg" style={{ background: 'var(--card-bg)' }}>
              ...
              <span className="text-sm font-medium" style={{ color: 'var(--primary-text)' }}>
                {t("productDetails.specifications.warranty")}
              </span>
            </div>
          </div>
        </div>
                ...
                {product.specifications && product.specifications.length > 0 ? (
                  <div className="grid gap-4">
                    {product.specifications.map((spec, idx) => (
                      <div key={idx} className="flex justify-between items-center py-3 border-b border-gray-200 dark:border-gray-700 last:border-b-0">
      ...
      {relatedProducts.length > 0 && (
        <section className="max-w-7xl mx-auto px-4 py-8">
          <h2 className="text-2xl font-bold mb-6" style={{ color: 'var(--primary-text)' }}>
            ...
            {relatedProducts.map((relatedProduct) => (
              ...
              >
                ...
                <div className="p-4 flex flex-col gap-2">
                  ...
                  <h3 className="font-semibold text-base truncate" style={{ color: 'var(--primary-text)' }}>
                    {relatedProduct.name?.[lang] ||
                      relatedProduct.name?.en ||
                      relatedProduct.name}
                  </h3>
