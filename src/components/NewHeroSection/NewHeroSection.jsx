...
import { useLanguage, getLocalizedText, getDirectionalClass, getTextAlignment } from '../../utils/languageUtils';
import 'swiper/css';
...
const NewHeroSection = () => {
  ...
  const getDiscountedPrice = (product) => {
    if (product.discountPercentage && product.discountPercentage > 0) {
      return (product.basePrice * (1 - product.discountPercentage / 100)).toFixed(2);
    }
  ...
  const transformProduct = (product) => {
    ...
    const productBrand = typeof product.brand?.name === 'object'
      ? getLocalizedText(product.brand.name, language, '')
      : product.brand?.name || '';
    
    const productCategory = typeof product.category?.name === 'object'
      ...
      <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
        <div className="relative">
          {transformedProducts.length > 0 ? (
            ...
            >
              {transformedProducts.map((product) => (
                <SwiperSlide key={product.id}>
                  <div 
                    onClick={() => handleProductClick(product.originalProduct || product)} 
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          {product.discountPercentage > 0 && (
                            <span className="text-sm text-gray-500 dark:text-gray-400 line-through">
                              {t('common.currency')}{Math.round(product.price / (1 - product.discountPercentage / 100))}
                            </span>
                          )}
                        </div>
                      </div>
            ...
            <div className={`text-center p-6 bg-white dark:bg-slate-800 rounded-3xl shadow-2xl ${getTextAlignment(isRTL, 'center')}`}>
        ...
        <div className={`text-center lg:text-left ${getTextAlignment(isRTL, 'left')}`}>
          <div className="space-y-6">
            <div className="flex items-center gap-3 mb-4">
              <span className="text-sm font-medium px-3 py-1 rounded-full bg-blue-100 dark:bg-blue-900 text-blue-800 dark:text-blue-200">
                {transformedProducts.length} {language === 'ar' ? 'منتج متاح' : 'Products Available'}
              </span>
            </div>
            ...
            >
              {language === 'ar' ? (
                <>
                  اكتشف الإلكترونيات <span className="bg-clip-text text-transparent"
                    style={{
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              <button 
                ...
                className={`inline-flex items-center gap-3 font-semibold text-lg px-8 py-4 rounded-full shadow-lg hover:shadow-xl transition-all duration-300 hover:-translate-y-1 group ${getDirectionalClass('', isRTL, 'flex-row-reverse', 'flex-row')}`}
                style={{
                  background: 'linear-gradient(to right, var(--accent-gradient-from, #2563eb), var(--accent-gradient-to, #7c3aed))',
                  color: 'white'
                }}
              >
                {language === 'ar' ? 'تسوق الآن' : (t('hero.shopNow') || 'Shop Now')}
                <svg 
                  width="16" 
                  height="16" 
                  viewBox="0 0 16 16" 
